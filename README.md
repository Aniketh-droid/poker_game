...
*Measured before the `observe()`-street bug described in Known Limitations was fixed. That fix has since been applied (with a regression test) but narrows rather than closes the gap - see below - so this table is still representative; re-run `python main.py` for fresh numbers.*
...
Together, these mean the Bayesian agent's edge is currently real but narrower and more conditional than the "star of the project" framing implied: it reliably exploits agents whose behavior resembles its hard-coded archetypes (or is unconditionally weak, like Random), and - even with item 1 fixed - still tends to lose to an opponent that plays sound EV-maximizing poker without matching any of those archetypes. Fixing item 2 properly is exactly the **Dynamic RL Opponent Modeling** item already listed below, and is the highest-leverage next step for this project.
